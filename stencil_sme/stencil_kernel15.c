/**
 * @file stencil_15x15_sme_optimized.c
 * @brief ARM SME optimized 15×15 stencil computation with multiple optimization strategies
 * @author ZHANGFAN
 * @date 2025/9/26
 * 
...
#define KERNEL_ELEMENTS (KERNEL_SIZE * KERNEL_SIZE)  // 225

#define ORIGINAL_SIZE 128
#define PADDING 7  // For 15×15 kernel
...
#define WEIGHT_COLS 16  // Weight matrix is 225×16
#define PADDED_STENCIL_COLS 240  // Padded to multiple of 16 for SME alignment
...
void initialize_data(float* input, float* kernel, float* original_data) {
    ...
    // Initialize kernel as averaging filter
...
void kernel2col_padded(const float* kernel, float** weight_A_out, float** weight_B_out) {
    int padded_rows = PADDED_STENCIL_COLS;  // 240
    
    float* weight_A = (float*)aligned_alloc(64, padded_rows * WEIGHT_COLS * sizeof(float));
    float* weight_B = (float*)aligned_alloc(64, padded_rows * WEIGHT_COLS * sizeof(float));
    ...
    memset(weight_A, 0, padded_rows * WEIGHT_COLS * sizeof(float));
    memset(weight_B, 0, padded_rows * WEIGHT_COLS * sizeof(float));
...
double cpu_stencil2row_sme_single(const float* matrix_A, const float* matrix_B,
    ...
    matmul_sme_single_tile(matrix_A, weight_A, result_A, stencil_rows, stencil_cols, WEIGHT_COLS);
    matmul_sme_single_tile(matrix_B, weight_B, result_B, stencil_rows, stencil_cols, WEIGHT_COLS);
...
double cpu_stencil2row_sme_4tiles(const float* matrix_A, const float* matrix_B,
    ...
    matmul_sme_4tiles(matrix_A, weight_A, result_A, stencil_rows, stencil_cols, WEIGHT_COLS);
    matmul_sme_4tiles(matrix_B, weight_B, result_B, stencil_rows, stencil_cols, WEIGHT_COLS);
...
double cpu_stencil2row_sme_4tiles_row_split(const float* matrix_A, const float* matrix_B,
    ...
    matmul_sme_4tiles_row_split(matrix_A, weight_A, result_A, stencil_rows, stencil_cols, WEIGHT_COLS);
    matmul_sme_4tiles_row_split(matrix_B, weight_B, result_B, stencil_rows, stencil_cols, WEIGHT_COLS);
...
void reorganize_padded_output(const float* result_padded, float* output, 
    ...
    for (int i = 0; i < OUTPUT_SIZE; i++) {
        ...
        for (int j = valid_start; j < valid_end && out_col < OUTPUT_SIZE; j += groupsize) {
            ...
            for (int k = 0; k < group_size_actual && out_col < OUTPUT_SIZE; k++) {
                output[i * OUTPUT_SIZE + out_col] = result_padded[result_idx * WEIGHT_COLS + k];
                out_col++;
            }
            result_idx++;
        }
...
int main() {
    printf("==========================================\n");
    printf("15×15 Stencil Computation - ARM SME Optimized\n");
    printf("==========================================\n");
